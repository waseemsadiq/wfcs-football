...
<body class="bg-background text-text-main font-sans antialiased min-h-screen flex flex-col">
    <header class="bg-surface/80 backdrop-blur-md border-b border-border py-4 mb-8 sticky top-0 z-50">
        <div class="max-w-[1200px] mx-auto px-6 w-full flex justify-between items-center">
            <a href="/admin" class="flex items-center no-underline">
                <img src="/images/logo-white.svg" alt="WFCS Football" class="h-16 w-16">
            </a>
            ...
            <nav class="hidden md:flex items-center space-x-6">
                ...
                <form method="POST" action="/logout" class="ml-8">
            ...
            <button id="mobile-menu-btn" class="md:hidden p-2 text-text-muted hover:text-primary transition-colors" aria-label="Open menu">
    ...
    <div id="sidebar-overlay" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden" onclick="closeSidebar()"></div>

    <!-- Mobile Sidebar -->
    <aside id="mobile-sidebar" class="fixed top-0 right-0 h-full w-64 bg-surface border-l border-border z-50 transform translate-x-full transition-transform duration-300 ease-in-out">
        <div class="flex flex-col h-full">
            <!-- Sidebar Header -->
            <div class="flex items-center justify-between p-6 border-b border-border">
                <span class="text-lg font-bold text-primary">Menu</span>
                <button onclick="closeSidebar()" class="p-2 text-text-muted hover:text-primary transition-colors" aria-label="Close menu">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="p-6 border-t border-border">
                <form method="POST" action="/logout" class="w-full">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(\Core\Auth::csrfToken()) ?>">
                    <button type="submit"
                        class="btn btn-sm bg-transparent border border-danger text-danger hover:bg-danger/10 w-full">Log out</button>
                </form>
            </div>
        </div>
    </aside>
    ...
    <footer class="mt-20 py-12 text-center text-text-muted border-t border-border text-sm">
        <div class="max-w-[1200px] mx-auto px-6 w-full">
            <p>&copy; <?= date('Y') ?> WFCS Football | App by <a href="https://www.waseemsadiq.com" target="_blank">Waseem Sadiq</a></p>
        </div>
    </footer>

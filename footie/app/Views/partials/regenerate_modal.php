...
<!-- Regenerate Modal -->
<div id="<?= htmlspecialchars($modalId) ?>" popover="manual"
    class="bg-surface p-10 rounded-md w-[90vw] max-w-lg shadow-glow border border-border h-fit max-h-[90vh] overflow-y-auto backdrop:bg-black/80 backdrop:backdrop-blur-sm m-auto">
    ...
    <form id="regenerateForm" method="POST" action="<?= htmlspecialchars($formAction) ?>">
        ...
        <div id="modal-fields">
            <div class="mb-6">
                ...
                <input type="date" id="modal-startDate" name="startDate"
                    value="<?= htmlspecialchars($competition['startDate'] ?? '') ?>" required aria-required="true"
                    class="form-input">
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="modal-frequency" name="frequency" required aria-required="true" class="form-input">
                    <option value="weekly" <?= ($competition['frequency'] ?? 'weekly') === 'weekly' ? 'selected' : '' ?>>
                        Weekly</option>
                    <option value="fortnightly" <?= ($competition['frequency'] ?? '') === 'fortnightly' ? 'selected' : '' ?>>Fortnightly</option>
                    <option value="monthly" <?= ($competition['frequency'] ?? '') === 'monthly' ? 'selected' : '' ?>>
                        Monthly</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="time" id="modal-matchTime" name="matchTime"
                    value="<?= htmlspecialchars(substr($competition['matchTime'] ?? '15:00', 0, 5)) ?>" required
...
<script>
    ...
    function handleModalClose() {
        if (isRegenerated) {
            ...
            document.getElementById('<?= htmlspecialchars($modalId) ?>').hidePopover();
        }
    }
